...
intros. destruct H, H0. simpl. rewrite H, H0.
  ...
  assert(P: forall (n : nat), I :: repeat I n = repeat I (S n)).
  {
    intros. simpl. reflexivity.
  }
  assert (P_NotL : forall (n : nat), NotL (repeat O n) = repeat I n).
  ...
  rewrite P_NotL.
  assert(H2: forall (n : nat), NotL (repeat I n) = repeat O n).
  {
    intros. induction n0.
    - reflexivity.
    - simpl. rewrite IHn0. reflexivity.
  ...
  assert (H3: forall (b : B) (n : nat), repeat b n ++ [b] = b :: repeat b n).
  {
    intros. induction n0.
    - simpl. reflexivity.
    - simpl. rewrite IHn0. reflexivity.
  ...
  assert (P_rev_2 : forall (b : B) (n : nat), rev (repeat b n) = repeat b n).
  {
      intros. induction n0.
      - reflexivity.
      - simpl. rewrite IHn0. rewrite H3. reflexivity.
  }

  assert(P2: forall (n : nat), AddLr (repeat I n) (repeat I n) I = repeat I n).
  {
  ...
  assert(B: AddL (repeat I n) (repeat I n) = repeat I (pred n) ++ [O]).
  {
    intros. induction n.
    - simpl. contradiction.
    - simpl. rewrite P. unfold AddL. rewrite P_rev_2. simpl. rewrite P2. rewrite P_rev_2.
      reflexivity.
  }

  assert(P3: forall (n : nat), NotL (repeat I n ++ [O]) = repeat O n ++ [I]).
  {
    intros. induction n0.
    - simpl. reflexivity.
    - simpl. rewrite IHn0. reflexivity.
  ...
  induction n.
  - simpl. contradiction.
  - simpl. rewrite P. rewrite B. simpl. rewrite P3.

  assert(H5: forall n, repeat O n ++ [I] = NotL (repeat I n) ++ [I]).
  {
    intros. induction n0.
    - simpl. reflexivity.
    - simpl. rewrite IHn0.
