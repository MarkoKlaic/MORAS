class Parser:
    ...
    def _parseLines(self):
        for (line, n) in self._ilines:
            l2 = line.split()
            if len(l2)>1 and l2[0] == "label" :
                self._labels.append(l2[1])
        lines = []
        for (line, n) in self._ilines:
    ...
    def _parseLine(self, line, n):
        ...
        if l[0] == "push":
            if len(l) == 3 and l[2].isnumeric():
                ...
                self._flag = False
                Parser._error("Parser", n, "Nije nenegativan cijeli broj");
                return ""
            ...
            if l[0] == "label" and len(l) == 2:
                return "//" + " ".join(l) + "\n" + self._label(l[1], n)
            elif l[0] == "goto" and len(l) == 2 and l[1] in self._labels:
                return "//" + " ".join(l) + "\n" + self._goto(l[1], n)
            elif l[0] == "goto" and len(l) == 2 and l[1] not in self._labels:
                self._flag = False
                Parser._error("Parser", n, "Goto label");
                return ""
            elif l[0] == "if-goto" and len(l) == 2 and l[1] in self._labels:
                return "//" + " ".join(l) + "\n" + self._ifgoto(l[1], n)
            elif l[0] == "if-goto" and len(l) == 2 and l[1] not in self._labels:
                self._flag = False
                Parser._error("Parser", n, "If-goto label");
                return ""
